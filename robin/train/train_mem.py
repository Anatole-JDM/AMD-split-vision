...
if __name__ == "__main__":
    ...
    match 'hpcfund':
        ...
        case x if 'hpcfund' in x:
            ...
            username = os.environ.get('USER')
            jobid = os.environ.get('SLURM_JOB_ID')
            jobid = int(jobid)+randint(0, 1000000)
            if jobid == None:
                jobid = randint(0, 1000000)

            while os.path.exists(f'/work1/aroger/{username}/miopen/{jobid}/{hostname}'):
                jobid += 1

            miopen_use_db_path = f'/work1/aroger/{username}/miopen/{jobid}/{hostname}'
            if not os.path.exists(miopen_use_db_path):
                os.makedirs(miopen_use_db_path) 
            os.environ['MIOPEN_USER_DB_PATH'] = miopen_use_db_path
            os.environ['MIOPEN_CUSTOM_CACHE_DIR'] = os.environ['MIOPEN_USER_DB_PATH']

            os.environ['WANDB_DIR'] = f'/work1/aroger/{username}/wandb_cache'
            os.environ['WANDB_MODE'] = 'offline'
            os.environ['TRANSFORMERS_CACHE'] = f'/work1/aroger/{username}/downloaded_models/hf_cache'
            os.environ['TRANSFORMERS_OFFLINE'] = '1'
            os.environ['HF_DATASETS_OFFLINE'] = '1'
            ...
            # wandb.init(project="your_project", dir=f'/work1/aroger/{username}/wandb_cache', mode="offline")

        case _:
            print(hostname)
            print('No cluster specific config, no enviroment variables set.')

    train(USE_FLASH_ATTN_2=USE_FLASH_ATTN_2, SET_SEED=SET_SEED)
