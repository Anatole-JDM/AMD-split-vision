...
from robin.mm_utils import tokenizer_image_token, expand2square, set_seed
...
from robin.model.multimodal_encoder.split_vision_tower import MultiVisionTower, split_shape, split_image
...
@dataclass
class DataArguments:
    split_shape: Optional[str] = field(default=None)
        ...
        if 'image' in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                if isinstance(processor, list):
                    split_shape = [int(self.data_args.split_shape[0]), int(self.data_args.split_shape[2])]
                    image_parts = split_image(image, split_shape)
                    image=[]
                    for proc, part, is_clip in zip(processor, image_parts, self.data_args.is_clip):
                        if is_clip:
                            image.append(proc.preprocess(part, return_tensors='pt')['pixel_values'][0])
                        else:
                            image.append(proc(part).unsqueeze(0))
                else:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if all(x is not None and split_shape(x) == split_shape(images[0]) for x in images):
...
def train(USE_FLASH_ATTN_2=False, SET_SEED=False):
    if SET_SEED:
        set_seed()
    ...
    if model_args.vision_tower is not None:
        ...
        if PRINT_MODEL: print(model_args)
        if training_args.output_dir is not None:
            if "2x2" in training_args.output_dir:
                split = [2,2]
            ...
            elif "3x2" in training_args.output_dir:
                split = [3,2]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "2x3" in training_args.output_dir:
                split = [2,3]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "3x3" in training_args.output_dir:
                split = [3,3]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "1x3" in training_args.output_dir:
                split = [1,3]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "3x1" in training_args.output_dir:
                split = [3,1]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "1x2" in training_args.output_dir:
                split = [1,2]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
            ...
            elif "2x1" in training_args.output_dir:
                split = [2,1]
                model.get_model().initialize_vision_modules(
                    model_args=model_args,
                    fsdp=training_args.fsdp,
        ...
        if isinstance(vision_tower, OpenCLIPVisionTower):
            ...
            vision_tower.device = training_args.device
            #idk about this bro
        elif isinstance(vision_tower, MultiVisionTower):
            data_args.is_clip = []
            
            for vt in vision_tower.vision_tower_list:
                
                if isinstance(vt, OpenCLIPVisionTower):
                    data_args.is_clip.append(False)
                    vt.to(training_args.device, dtype=torch.float16)
                elif isinstance(vt, CLIPVisionTower):
                    data_args.is_clip.append(True)
                    vt.to(training_args.device, dtype=torch.float16)
                elif isinstance(vt, TimmVisionTower):
                    data_args.is_clip.append(False)
                    vt.to(training_args.device, dtype=torch.float16)

                else:
                    raise ValueError(f'Unknown vision tower type: {type(vt)}')
